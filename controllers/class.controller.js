...
const createClass = async (req, res, next) => {
  ...
  try {
    ...
    if (!checkIdMongo(teacher)) {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Vui lòng truyền đúng định dạng ObjectId');
    }
...
const updateClassById = async (req, res, next) => {
  ...
  try {
    if (JSON.stringify(updateBody) == '{}') {
      throw new ApiError(httpStatus.BAD_REQUEST, 'Vui lòng điền đủ thông tin');
    }
...
const leaveClass = async (req, res, next) => {
  ...
  try{
    ...
    if(classroom.students?.includes(studentId)){
      ...
      throw new ApiError(httpStatus.NOT_FOUND, 'Không tồn tại lớp học');
    }
